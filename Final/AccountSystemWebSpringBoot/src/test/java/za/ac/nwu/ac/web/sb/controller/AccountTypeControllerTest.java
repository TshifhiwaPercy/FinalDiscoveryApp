...
public class AccountTypeControllerTest {
    ...
    public void updateAccountTypeWithNoOptionalDate() throws Exception {
        ...
        MvcResult mvcResult = mockMvc.perform(put((String.format("%s/%s",
                ...
                .andExpect(status().isOk())
                .andReturn();
        verify(modifyAccountTypeFlow, times(1)).updateAccountType(eq("PLAY"), eq("The new Play account type name"), eq(null));assertEquals(expectedResponse, mvcResult.getResponse().getContentAsString());
    }
    @Test
